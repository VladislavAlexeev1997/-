	...
	<body>
		<table class="struct menu">
			<tr>
				...
				<td class="element element_not_right">
					<a href = "publication_articles_editor.php?login=<?php echo $_GET['login']?>" class="select_menu">Опубликованные статьи</a>
				</td>
				<td class="element element_not_right">
  		...
  		<div class="articles">
			<center><table class="articlestable">
				...
				<?php
					...
					if (!$con) {
						...
						$articles = mysqli_query($con, "SELECT * FROM (SELECT article_photo.file_name, 
							article.article_id, article.article_name, article.date_public, article.time_public 
							FROM article, article_photo WHERE article.editor_login = '".$_GET['login']."' AND 
				...
				<tr>
					...
					<td width="45%" class="article_element"><a class="article_name" href="article_only.php?action=articles_editor&login=<?php echo $_GET['login']?>&article_id=<?php echo $row["article_id"]?>"><?php echo $row["article_name"];?></a></td>
					<td width="15%" class="article_element"><?php echo $row["date_public"]." ".$row["time_public"];?></td>
					<td class="article_element"><a class="edit_button" href="edit_article.php?login=<?php echo $_GET['login']?>&article_id=<?php echo $row["article_id"]?>">Редактировать</a></td>
				</tr>
				<?php
							$index++;
						}
