...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Главная страница подписчика</title>
		<style>
			body {
				background: #F5F5DC;
				margin: 0;
		...
		<table class="struct menu">
			<tr>
				<td class="element element_not_right">
					<a href = "start_subscr_page.php?login=<?php echo $_GET['login']?>" class="select_menu">Главная</a>
				</td>
				<td class="element element_not_right">
		...
		<div class="news">
			<div class="news_block">
				...
				<div class="last_news">
					...
					<p><a class="head_last_news" href="article_only.php?action=start_subscr&login=<?php echo $_GET['login']?>&article_id=<?php echo $row["article_id"]?>"><?php echo $row["article_name"];?></a></p>
				</div>
				<?php 
					}
					$index++;
  		...
  		<div class="mainnew">
			<?php
				$editor = mysqli_query($con, "SELECT subscriber.surname, subscriber.name, subscriber.patronymic
						FROM subscriber WHERE subscriber.login = '".$_GET['login']."';");
				$row = mysqli_fetch_array($editor);
			?>
   			<p class="welcome">Здравствуйте, <?php echo $row["name"]." ".$row["patronymic"]." ".$row["surname"];?>!</p>
			<?php
			...
			<div class="main_new">
				...
				<div class="main">
					...
					<p><a class="head_main_news" href="article_only.php?action=start_subscr&login=<?php echo $_GET['login']?>&article_id=<?php echo $row["article_id"]?>"><?php echo $row["article_name"];?></a></p>
